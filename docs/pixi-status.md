...
- `src/ui/layouts/appShell.ts` provides AppShell, TopBar, ContentHost, BottomBar, and BottomSheetHost. `src/ui/button.ts` wraps `@pixi/ui` `FancyButton` with project tokens plus shared UI bounds and button hit helpers used by AppShell and Scene Index. Scene Index and Design System use ContentHost for UI/content panels; gameplay/world scenes keep world objects in `layers.world` and use AppShell for HUD/navigation/control slots.
