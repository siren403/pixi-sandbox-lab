...
- `src/ui/layouts/appShell.ts` provides AppShell, TopBar, ContentHost, BottomBar, BottomSheetHost, and shared button hit helpers. Scene Index and Design System use ContentHost for UI/content panels; gameplay/world scenes keep world objects in `layers.world` and use AppShell for HUD/navigation/control slots.
