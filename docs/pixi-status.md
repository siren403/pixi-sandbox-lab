...
- Scene switches accept either the legacy source string or `{ source, args }`; active scene metadata is exposed as `ctx.scene` so target scenes can read one-shot transition args.
