...
- scene 전환 시 기존 화면 위로 runtime-owned loading overlay가 fade-in으로 덮이고, 새 scene load 완료 후 fade-out으로 로드된 화면을 드러낸다.
- loading overlay는 progress bar와 loop animation을 포함하고, 연출 확인을 위해 최소 500ms 유지한다.
- `window.__pixiRuntimeState`는 E2E용으로 loading 상태, loading phase, scene switch 수, loading overlay 표시 수, 마지막 loading duration, progress, overlay alpha와 최대 alpha를 노출한다.
