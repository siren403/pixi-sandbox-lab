...
- 앱은 intro scene에서 시작하며, 사용자의 tap/click 또는 Enter/Space 입력 후 boot scene으로 전환한다.
- scene unload 이후 새 scene load 전까지 runtime-owned loading overlay를 `debug-layer`에 표시한다.
- loading overlay는 연출 확인을 위해 최소 500ms 유지한다.
- `window.__pixiRuntimeState`는 E2E용으로 loading 상태, scene switch 수, loading overlay 표시 수, 마지막 loading duration을 노출한다.
